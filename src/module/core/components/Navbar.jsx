...
export default function Navbar() {
  ...
  return (
    <div className={`navbar fixed top-0 left-0 right-0 z-10 w-screen transition-bg ${showNavbar ? 'translate-y-0' : '-translate-y-full'} pr-8 ${DarkMode ? "bg-white/40" : "bg-black/40"} backdrop-blur-md drop-shadow-md`}>
      <div className="display-none flex-none">
        <SideBar />
      </div>
      {/* Logo y nombre */}
      ...
      <div className='flex items-center gap-2'>
        <label className="swap swap-rotate">
          ...
          <svg className="swap-off h-10 w-10" fill="#f8da5b" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
        ...
        <div className="dropdown dropdown-end">
          ...
          <ul tabIndex={0} className={`menu menu-sm dropdown-content ${DarkMode ? "bg-primary" : "bg-secondary"} rounded-box z-1 mt-3 w-52 p-2 shadow`}>
            {isAuthenticated
              ? <>
                <li>
                  <a className={`justify-between ${DarkMode ? "text-secondary" : "text-primary"}`} onClick={() => navigate(`/profile/${authUser?.id}`)}>
                    Perfil
                  </a>
                </li>
